...
class LQDataset(YOLODataset):
    ...
    def _get_files(self, im_dirs, re=["*"], prefix="image"):
        """Get image files.
        im_dir
        └──pic1.jpg
        └──pic2.jpg
        ...
        Args:
            im_dirs (List[str] | str): The image directories.
        ...
        for im_dir in im_dirs:
            if os.path.isdir(im_dir):
                ...
                LOGGER.info(colorstr(f"{Prefix} dir: ") + f"'{im_dir}' is not a valid dir.")
        return im_files
    ...
    def update_labels_info(self, label, neg=False):
        ...
        if neg:
            ...
            label["instances"] = Instances(
                bboxes=np.zeros((0, 4), dtype=np.float32), keypoints=keypoints, segments=segments
            )
            return label
