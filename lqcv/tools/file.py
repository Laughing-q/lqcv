...
def remove_extra_files(more_dir, less_dir, target_dir=None, reverse=False, copy=False):
    ...
    Args:
        ...
        copy (bool): If copy=True, then copy the extra files to `target_dir`, otherwise move them.
    """
    ...
    for name in names:
        ...
        if target_dir:
            shutil.copy(file, target_dir) if copy else shutil.move(file, target_dir)
        else:
            os.remove(file)
